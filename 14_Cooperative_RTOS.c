...
int rtosScheduler()
{
      tcb[taskCurrent].cpucount = tcb[taskCurrent].cpucount + counter;
      total_ticks = total_ticks + counter;
      counter = 0;
      bool ok;
...
void pendSvIsr()
{
    ...
    {
     __asm("      MOV32 R0,#0x01000000");    //Pushing extra registers
     __asm("      PUSH {R0}");               //1
...
// -----------------------------------------------------------------------------
// shell input commands                                           REFERENCE FROM MY EE5314 PROJECT
// -----------------------------------------------------------------------------
